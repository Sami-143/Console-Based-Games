...
// constant 5 products for the sale
...
// array for use of product type
string productType[productCount];
...
void addNewProduct()
{
    ...
    cout << "ENTER THE STOCK OF THE PRODUCT :";
    cin >> pStock;
    totalProductName[pCount] = pName;
    totalProductPrice[pCount] = pPrice;
    totalProductStock[pCount] = pStock;
    pCount++;
}
void topHeader()
{
...
int customerMenu()
{
    ...
    cout << " 5.Remove Product from the cart" << endl;
...
void loadData()
{
    ...
    while (getline(myfile, name) && getline(myfile, password) && getline(myfile, role))
    {
        ...
        // users[usersCounter] = parseRecord(record, 1);
        // passwords[usersCounter] = parseRecord(record, 2);
        // roles[usersCounter] = parseRecord(record, 3);

        // feedbackNames[x] = parseRecord(record, 7);
        // feedbacks[x] = parseRecord(record, 8);
        // colours[x] = parseRecord(record, 9);
        // sizeAvailable[x] = stoi(parseRecord(record, 10));
        users[usersCounter] = name;
        passwords[usersCounter] = password;
        roles[usersCounter] = role;
        // passwords[usersCounter] = parseRecord(record, 2);
...
void loadData1()
{
    ...
    {
        totalProductName[x] = parseRecord(record, 1);
        totalProductPrice[x] = stoi(parseRecord(record, 2));
        totalProductStock[x] = stoi(parseRecord(record, 3));
...
void loadData2()
{
    ...
    while (getline(myfile, record))
    {
        ...
        quantityOfProduct[x] = stoi(parseRecord(record, 2));
        SalesPrice[x] = stoi(parseRecord(record, 3));
